...
def make_title(
    ...
    *,
    ...
    ma   = smoothing_suffix(ma_window)  # <<< NEW

    head = f"{who} - {what}" if who else what
    tail_bits = [geo, pt, span, ma]     # <<< include suffix here
...
try:
    if CACHE.exists():
        ...
        # Cache missing: stop with a clear instruction instead of rebuilding from Excel
        st.error("Cache file missing: `.cache/long.parquet`")
        st.info(
            "Build it locally with:\n\n"
            "`python tools/build_cache.py`\n\n"
            "Then commit/upload the entire `.cache` folder (including "
            "`long.parquet` and `.manifest.json`)."
        )
        st.stop()
...
required_cols = {"yyyymm", "total_sales", "country"}
missing = required_cols - set(df.columns)
if missing:
    st.set_option("client.showErrorDetails", True)
    st.error(f"Data is missing required column(s): {sorted(missing)}")
    st.stop()

# Create optional columns if absent
for col in ["maker_brand", "group", "powertrain_simplified"]:
    if col not in df.columns:
        df[col] = ""

# Normalize maker_brand
df["maker_brand"] = df["maker_brand"].astype(str).str.strip().str.upper()

# Flatten OEM group map
flat_oem_map = {
    raw: clean
    for clean, raw_list in oem_group_map.items()
    for raw in raw_list
}

# Apply OEM standardization safely
df["group_standardized"] = (
    df["group"].astype(str).map(flat_oem_map).fillna(df["group"]).astype(str).str.strip()
)

# Build brand→OEM mapping if possible
brand_to_oem = {}
if {"maker_brand", "group_standardized"}.issubset(df.columns):
    brand_to_oem = (
...
# ---- Moving Average control (global for time-dependent charts) ----
MA_CHOICES = {"Raw": None, "3-Month MA": 3, "6-Month MA": 6, "12-Month MA": 12}
selected_ma_label = st.sidebar.selectbox("Moving Average", list(MA_CHOICES.keys()), index=0)
selected_ma_window = MA_CHOICES[selected_ma_label]
...
applied_ma_window = applied.get("ma_window", None)
applied_ma_label  = applied.get("ma_label", "Raw")
...
def month_span_len(applied_month_range: tuple[str,str] | None) -> int:
    """Inclusive count of months in ('YYYYMM','YYYYMM')."""
    if not applied_month_range: 
        return 0
    s, e = map(int, applied_month_range)
    sy, sm = divmod(s, 100); ey, em = divmod(e, 100)
    return (ey - sy) * 12 + (em - sm) + 1

def apply_trailing_ma(
    df: pd.DataFrame,
    *,
    group_cols: list[str],
    date_col: str,
    ...
    """
    Strict trailing MA: include current month, require full window (min_periods=window).
    Adds f"{value_col}_ma" aligned with the original rows.
    Handles categoricals with unused categories to avoid empty-group rolling errors.
    """
    if not window or window < 2 or df.empty:
        out = df.copy()
        out[f"{value_col}_ma"] = np.nan
        return out

    out = df.copy()

    # 1) Drop unused categories on all grouping cols (prevents empty groups)
    for c in group_cols:
        if c in out.columns and pd.api.types.is_categorical_dtype(out[c]):
            out[c] = out[c].cat.remove_unused_categories()

    # 2) Sort for stable rolling
    out = out.sort_values(group_cols + [date_col])

    # 3) Safe rolling (guard s.empty)
    def _safe_roll(s: pd.Series) -> pd.Series:
        if s.empty:
            return pd.Series(np.nan, index=s.index)
        return s.rolling(window=window, min_periods=window).mean()

    # 4) observed=True avoids empty categorical groups entirely
    out[f"{value_col}_ma"] = (
        out.groupby(group_cols, observed=True, group_keys=False)[value_col]
           .transform(_safe_roll)
    )
    return out

def first_common_valid_ma(df: pd.DataFrame, *, group_col: str, date_col: str, value_ma_col: str) -> pd.DataFrame:
    """
    For stacked bars: start where ALL series have a valid MA.
    Returns df filtered to >= max(first_valid_index per series).
    """
    if df.empty: 
        return df
    firsts = (
        df.dropna(subset=[value_ma_col])
          .groupby(group_col)[date_col].min()
    )
    if firsts.empty or len(firsts) == 0:
        return df.iloc[0:0]
    common_start = firsts.max()
    return df[df[date_col] >= common_start].copy()

def smoothing_suffix(n: int | None) -> str:
    return "" if not n or n < 2 else f"{n}-Month Moving Average"

def _effective_ma_win(applied_ma_window, applied_month_range) -> int | None:
    """
    Global rule:
    - Single-month -> raw
    - If selected span < N -> raw + yellow note
    - Else -> N
    """
    if not applied_ma_window:
        return None
    # single-month always raw
    if applied_month_range and applied_month_range[0] == applied_month_range[1]:
        show_warning_black(f"Need at least {int(applied_ma_window)} months for the selected smoothing.")
        return None
    need = int(applied_ma_window)
    have = month_span_len(applied_month_range)
    if have < need:
        show_warning_black(f"Need at least {need} months for the selected smoothing.")
        return None
    return need
...
def render_sales_internal(
    ...
    def _render_one_powertrain(df_slice: pd.DataFrame, pts_for_title: list | None):
        ...
        if not single_month:
            eff_win = _effective_ma_win(applied_ma_window, applied_month_range)
            data_for_line = view_df.sort_values("period_dt").copy()
            y_col = "total_sales"
            if eff_win:
                data_for_line = apply_trailing_ma(
                    data_for_line, group_cols=[PLOT_COL], date_col="period_dt",
                    value_col="total_sales", window=eff_win
                )
                y_col = "total_sales_ma"
            ...
            fig_line = px.line(
                ...
                # attach per-row custom data here (trace-specific)
                custom_data=["total_sales"]
            )

            if eff_win:
                fig_line.update_traces(
                    hovertemplate="Month: %{x|%m/%Y}<br>Smoothed: %{y:,.0f}<br>Actual: %{customdata[0]:,.0f}<extra></extra>"
                )
            else:
                fig_line.update_traces(
                    hovertemplate="Month: %{x|%m/%Y}<br>Units: %{y:,.0f}<extra></extra>"
                )
        ...
        if single_month:
            ...
            # base data (raw monthly, by segment)
            bar_df = view_df.sort_values("period_dt").copy()
            y_col = "total_sales"

            if eff_win:
                # add moving average per segment and trim to first month where ALL have a value
                bar_df = apply_trailing_ma(
                    bar_df, group_cols=[PLOT_COL], date_col="period_dt",
                    value_col="total_sales", window=eff_win
                )
                bar_df = first_common_valid_ma(
                    bar_df, group_col=PLOT_COL, date_col="period_dt", value_ma_col="total_sales_ma"
                )
                y_col = "total_sales_ma"

            # ---------- build the chart ----------
            fig_bar = px.bar(
                bar_df, x="period_dt", y=y_col, color=PLOT_COL, barmode="stack",
                color_discrete_map=color_map,
                category_orders={PLOT_COL: segments},
                template="plotly_white",
                labels={"total_sales": "Unit Sales", "period_dt": "Month", PLOT_COL: PLOT_COL.title()},
            ...
            # Per-bar hover: (MA) + Actual (raw)
            if eff_win:
                fig_bar.update_traces(
                    customdata=bar_df[["total_sales"]],
                    hovertemplate=(
                        "Month: %{x|%m/%Y}<br>%{fullData.name}: %{y:,.0f} (MA)"
                        "<br>Actual: %{customdata[0]:,.0f}<extra></extra>"
                    )
                )
            else:
                # Raw mode: only show the raw value for each bar
                fig_bar.update_traces(
                    hovertemplate="Month: %{x|%m/%Y}<br>%{fullData.name}: %{y:,.0f}<extra></extra>"
                )

            # ---------- unified bottom totals ----------
            # We want the totals aligned to the months actually present in bar_df after any trimming.
            months_shown = (
                bar_df[["period_dt"]]
                .drop_duplicates()
            ...
            # Actual total per month (raw)
            tot_actual = (
                view_df.groupby("period_dt", as_index=False)["total_sales"].sum()
                .rename(columns={"total_sales": "total_actual"})
                .merge(months_shown, on="period_dt", how="right")   # align to months in the plot
                .sort_values("period_dt")
            )

            # Ave (MA) total per month – only when smoothing is on
            if eff_win:
                tot_ma = (
                    bar_df.groupby("period_dt", as_index=False)[y_col].sum()
                    .rename(columns={y_col: "total_ma"})
                    .sort_values("period_dt")
                )

                # invisible MA total trace (for unified tooltip)
                fig_bar.add_scatter(
                    x=tot_ma["period_dt"],
                    y=tot_ma["total_ma"],
                    mode="lines",
                    ...
                    opacity=0,
                    showlegend=False,
                    hovertemplate="Ave: %{y:,.0f}<extra></extra>",
                )

            # invisible Actual total trace (for unified tooltip)
            fig_bar.add_scatter(
                x=tot_actual["period_dt"],
                y=tot_actual["total_actual"],
                mode="lines",
                line=dict(width=0),
                opacity=0,
                showlegend=False,
        ...
        if (not single_month) and pts_for_title and len(pts_for_title) > 1:
            ...
            eff_win = _effective_ma_win(applied_ma_window, applied_month_range)
            pt_view = monthly_pt.sort_values("period_dt").copy()
            y_col = "total_sales"
            if eff_win:
                pt_view = apply_trailing_ma(
                    pt_view, group_cols=["powertrain_simplified"], date_col="period_dt",
                    value_col="total_sales", window=eff_win
                )
                y_col = "total_sales_ma"
            ...
            fig_line_pt = px.line(
                ...
                title=make_title(
                    ...
                    powertrains=pts_for_title,
                    where_override="Global" if is_global else None,
                    ma_window=eff_win,   # add MA suffix
                ),
                # IMPORTANT: attach row-level raw units so each trace gets the correct "Actual"
                custom_data=["total_sales"]
            )
            ...
            # Dual hover when smoothed
            if eff_win:
                fig_line_pt.update_traces(
                    hovertemplate=(
                        "Month: %{x|%m/%Y}<br>Smoothed: %{y:,.0f}"
                        "<br>Actual: %{customdata[0]:,.0f}<extra></extra>"
                    )
                )
            else:
...
def render_sales_competitive(
    ...
    if not single_month:
        ...
        if not line_df.empty:
            eff_win = _effective_ma_win(applied_ma_window, applied_month_range)
            data_for_line = line_df.sort_values("period_dt").copy()
            y_col = "total_sales"
            if eff_win:
                data_for_line = apply_trailing_ma(
                    data_for_line, group_cols=["OEM_adj"], date_col="period_dt",
                    value_col="total_sales", window=eff_win
                )
                y_col = "total_sales_ma"
            ...
            fig_line = px.line(
                ...
                custom_data=["total_sales"]   # attach here
            )
            ...
            if eff_win:
                ...
                fig_line.update_traces(hovertemplate="Month: %{x|%m/%Y}<br>Units: %{y:,.0f}<extra></extra>")
            fig_line.update_layout(
        ...
        if eff_win:
            bar_df = apply_trailing_ma(
                bar_df, group_cols=["OEM_adj"], date_col="period_dt",
                value_col="total_sales", window=eff_win
            )
            bar_df = first_common_valid_ma(
                bar_df, group_col="OEM_adj", date_col="period_dt", value_ma_col="total_sales_ma"
            )
            y_col = "total_sales_ma"

        # period totals based on the plotted y_col (raw or MA)
        bar_df["period_total_plot"] = bar_df.groupby("period_dt")[y_col].transform("sum")

        fig_bar = px.bar(
            bar_df, x="period_dt", y=y_col, color="OEM_adj", barmode="stack",
            ...
            custom_data=["period_total_plot", "total_sales"]  # total for tooltip + raw value for dual hover
        )

        if eff_win:
            fig_bar.update_traces(
                hovertemplate=(
                    "%{x|%m/%Y}<br>%{fullData.name}: %{y:,.0f} (MA)"
                    "<br>Actual: %{customdata[1]:,.0f}"
                    "<br>Total: %{customdata[0]:,.0f}<extra></extra>"
                )
            )
        else:
            fig_bar.update_traces(
                hovertemplate=(
                    "%{x|%m/%Y}<br>%{fullData.name}: %{y:,.0f}"
                    "<br>Total: %{customdata[0]:,.0f}<extra></extra>"
                )
            )
        ...
        if len(chosen_pts) > 1:
            ...
            if eff_win:
                ...
                y_col = "total_sales_ma"
                # strict window: drop pre-window NaNs so the line starts at first valid MA
                pt_view = pt_view.dropna(subset=[y_col])

            fig_pt = px.line(
                pt_view,
                x="period_dt", y=y_col, color="powertrain_simplified",
                template="plotly_white",
                labels={"total_sales":"Unit Sales", "period_dt":"Month", "powertrain_simplified":"Powertrain"},
                title=make_title(
            ...
            if eff_win:
                fig_pt.update_traces(
                    customdata=pt_view[["total_sales"]],
                    hovertemplate="Month: %{x|%m/%Y}<br>Smoothed: %{y:,.0f}<br>Actual: %{customdata[0]:,.0f}<extra></extra>"
                )
            else:
                fig_pt.update_traces(hovertemplate="Month: %{x|%m/%Y}<br>Units: %{y:,.0f}<extra></extra>")
...
def render_share_internal(
    ...
    if not single_month:
        ...
        eff_win = _effective_ma_win(applied_ma_window, applied_month_range)
        line_df = view.dropna(subset=["share"]).sort_values(["period_dt", PLOT_COL]).copy()
        y_col = "share"
        if eff_win:
            line_df = apply_trailing_ma(
                line_df, group_cols=[PLOT_COL], date_col="period_dt",
                value_col="share", window=eff_win
            )
            y_col = "share_ma"

        fig_line = px.line(
            line_df, x="period_dt", y=y_col, color=PLOT_COL,
            ...
            # Always carry raw share + units for hover
            custom_data=["share", "num", "den"],
        )
        if eff_win:
            fig_line.update_traces(
                hovertemplate=(
                    "Month: %{x|%m/%Y}<br>Smoothed: %{y:.1%}"
                    "<br>Actual: %{customdata[0]:.1%}"
                    "<br>OEM Units: %{customdata[1]:,.0f}"
                    "<br>Market Units: %{customdata[2]:,.0f}<extra></extra>"
                )
            )
        else:
            fig_line.update_traces(
                hovertemplate=(
                    "Month: %{x|%m/%Y}<br>Share: %{y:.1%}"
                    "<br>OEM Units: %{customdata[1]:,.0f}"
                    "<br>Market Units: %{customdata[2]:,.0f}<extra></extra>"
                )
            )
        ...
        if len(chosen_pts) > 1:
            ...
            eff_win = _effective_ma_win(applied_ma_window, applied_month_range)
            pt_line = pt_share.dropna(subset=["share"]).sort_values("period_dt").copy()
            y_col = "share"
            if eff_win:
                pt_line = apply_trailing_ma(
                    pt_line, group_cols=["powertrain_simplified"], date_col="period_dt",
                    value_col="share", window=eff_win
                )
                y_col = "share_ma"
            ...
            if eff_win:
                # Keep num/den hover the same; add smoothed vs actual via y/customdata if needed,
                # but since % is derived, we keep OEM/Market units in customdata as you already do.
                fig_pt.update_traces(
                    hovertemplate=("Month: %{x|%m/%Y}<br>Smoothed: %{y:.1%}"
                                "<br>OEM Units: %{customdata[0]:,.0f}"
                                "<br>Market Units: %{customdata[1]:,.0f}<extra></extra>")
                )
            else:
                fig_pt.update_traces(
                    hovertemplate=("Month: %{x|%m/%Y}<br>Share: %{y:.1%}"
                                "<br>OEM Units: %{customdata[0]:,.0f}"
                                "<br>Market Units: %{customdata[1]:,.0f}<extra></extra>")
                )
...
def render_share_competitive(
    ...
    if not single_month:
        ...
        if not line_df.empty:
            ...
            if eff_win:
                data_for_line = apply_trailing_ma(
                    data_for_line, group_cols=["group_standardized"], date_col="period_dt",
                    value_col="share", window=eff_win
                )
                y_col = "share_ma"

            fig_line = px.line(
                data_for_line, x="period_dt", y=y_col, color="group_standardized",
            ...
            if eff_win:
                fig_line.update_traces(
                    customdata=np.c_[data_for_line["total_sales"], data_for_line["market_total"]],
                    hovertemplate=("Month: %{x|%m/%Y}<br>%{fullData.name}: %{y:.1%}"
                                "<br>OEM Units: %{customdata[0]:,.0f}"
                                "<br>Market Units: %{customdata[1]:,.0f}<extra></extra>")
                )
            else:
