...
def update_region():
    # take map of country to region / reverse if needed
    # update:
    ...
    st.session_state["region_selectbox"] = country_to_region[selected_country]  # Example: update another widget
...
yopt = sorted(df_region["year"].astype(int).unique().tolist())
year_options = ["All"] + sorted(df_region["year"].astype(int).unique().tolist())


# change this from selct box to be a slidebar



# year options would be a string value of dates...

# instead will return 2 values, the first and the last date
...
# ============================================================================
    ...
    if year_range[0] == year_range[1]:
        df_region_year = df_region[df_region["year"] == int(year_range[1])].copy()
    else:
        df_region_year = df_region[df_region["year"].isin(range(int(year_range[0]), int(year_range[1]) + 1))].copy()
